                    ...
                    else if (ISID(lex))
                    {
                        ...
                        *last = (TEMPLATESELECTOR*)Alloc(sizeof(TEMPLATESELECTOR));
                        if (strsym->templateLevel)
                            strsym = strsym->maintemplate;
                        (*last)->sym = strsym;
                        if (strsym->templateLevel)
                        {
                            (*last)->isTemplate = true;
                            (*last)->templateParams = strsym->templateParams->p->bySpecialization.types ? strsym->templateParams->p->bySpecialization.types : strsym->templateParams->next;
                        }
                        last = &(*last)->next;
                        *last = (TEMPLATESELECTOR*)Alloc(sizeof(TEMPLATESELECTOR));
    ...
    while (1)
    {
        switch (tp->type)
        {
            ...
            case bt_templateselector:
            {
                ...
                {
                    if (!tp->sp->templateSelector->next->sym->instantiated)
                    {
...
static bool checkNonTypeTypes(TEMPLATEPARAMLIST* params, TEMPLATEPARAMLIST* enclosing)
{
    while (params)
    {
        ...
        {
            TYPE *tp2 = params->p->byNonType.tp;
            if (tp2->type == bt_templateselector)
            {
                if (tp2->sp->templateSelector->next->templateParams)
                {
                    tp2->sp->templateSelector->next->sym = tp2->sp->templateSelector->next->sym->maintemplate;
                    TEMPLATEPARAMLIST *params = tp2->sp->templateSelector->next->templateParams;
                    while (params)
                    {
                        TEMPLATEPARAMLIST *scan = enclosing;
                        while (scan)
                        {
                            if (params->argsym && enclosing->argsym && !strcmp(params->argsym->name, enclosing->argsym->name))
                            {
                                params->p->byClass.val = enclosing->p->byClass.val;
                                break;
                            }
                            scan = scan->next;
                        }
...
static bool TemplateDeduceFromArg(TYPE* orig, TYPE* sym, EXPRESSION* exp, bool byClass, bool allowSelectors)
{
    ...
    if (basetype(orig)->type == bt_rref)
    {
        ...
        {
            if (!ispointer(sym) && (lvalue(exp) || basetype(sym)->type != bt_rref && !basetype(sym)->rref))
            {
            ...
            */
            ...
            switch (dest->p->type)
            {
                case kw_typename:
                {
                    ...
                    if (isstructured(dest->p->byClass.val) && basetype(dest->p->byClass.val)->sp->templateParams)
                    {
                        TEMPLATEPARAMLIST *s = basetype(dest->p->byClass.val)->sp->templateParams;
                        while (s)
                        {
                            if (s->argsym)
                            {
                                TEMPLATEPARAMLIST *d = declareSym->templateParams->next;
                                while (d)
                                {
                                    if (!strcmp(d->argsym->name, s->argsym->name))
                                    {
                                        d->p->byClass.val = s->p->byClass.val;
                                        break;
                                    }
                                    d = d->next;
                                }
...
static SYMBOL* ValidateClassTemplate(SYMBOL* sp, TEMPLATEPARAMLIST* unspecialized, TEMPLATEPARAMLIST* args)
{
    ...
    if (nparams)
    {
        ...
        if (initial)
            rv = NULL;
        TEMPLATEPARAMLIST *origArgs = args;
        primary = params;
        while (primary)
        {
                    ...
                    if (params->p->byClass.val)
                    {
                        ...
                        case kw_typename:
                            if (params->p->byClass.dflt->type != bt_templateparam &&
                                ...
                                if (!isstructured(params->p->byClass.val) || !isstructured(params->p->byClass.dflt) || !basetype(params->p->byClass.dflt)->sp->parentTemplate)
                                {
                                    rv = nullptr;
                                }
                                else if (basetype(params->p->byClass.dflt)->sp->instantiated || basetype(params->p->byClass.val)->sp->parentTemplate != basetype(params->p->byClass.dflt)->sp->parentTemplate)
                                {
                                    rv = nullptr;
                                }
                            break;
        ...
        // for SFINAE, have to check that any templateselectors are valid...
        if (templateNestingCount)
        {
            args = origArgs;
            while (args)
            {
                if (args->p->type == kw_typename)
                {
                    if (args->p->byClass.dflt && args->p->byClass.dflt->type == bt_templateselector && !args->p->byClass.dflt->sp->templateSelector->next->isTemplate)
                    {
                        TYPE *hold = args->p->byClass.val;
                        args->p->byClass.val = args->p->byClass.dflt;
                        TYPE *tp = SynthesizeType(args->p->byClass.dflt, origArgs, false);
                        args->p->byClass.val = hold;
                        if (tp->type == bt_any)
                            return false;
